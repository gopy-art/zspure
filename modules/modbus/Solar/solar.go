...
func (s *Solar) SetCategory(category ...string) {
	s.Category = model.Category.Industrial()
}

func (s *Solar) SetDeviceName(device ...string) {
	s.DeviceName = "Solar"
}

func (s *Solar) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.mei_response.objects.vendor": "Solare"},
	}
}

func (s *Solar) Filters(banner map[string]interface{}) bool {
	...
	if val, ok := banner["mei_response"].(map[string]interface{})["objects"].(map[string]interface{})["vendor"].(string); ok {
		if strings.Contains(strings.ToLower(val), "solare") {
			return true
		}
	}
	return false
...
func (s *Solar) DeviceScan(banner map[string]interface{}) bool {
	s.ExtraInfo.NewExtraInfo()
	s.ExtraInfo.SetExtraInfo("manufacturer", "Solar Log")
...
func (s *Solar) CveScan(els *handler.Elastic) {
...
func (s *Solar) PrintInfo() string { return model.Category.Industrial() + " | Solar" }

func (s *Solar) Result() model.ModuleStructure {

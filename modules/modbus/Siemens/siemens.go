...
func (s *Siemens) SetCategory(category ...string) {
	s.Category = model.Category.Industrial()
}

func (s *Siemens) SetDeviceName(device ...string) {
	s.DeviceName = "Siemens"
}

func (s *Siemens) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.mei_response.objects.vendor": "Siemens"},
	}
}

func (s *Siemens) Filters(banner map[string]interface{}) bool {
	...
	if val, ok := banner["mei_response"].(map[string]interface{})["objects"].(map[string]interface{})["vendor"].(string); ok {
		if strings.Contains(strings.ToLower(val), "siemens") {
			return true
		}
	}
	return false
...
func (s *Siemens) DeviceScan(banner map[string]interface{}) bool {
...
func (s *Siemens) CveScan(els *handler.Elastic) {
	var CVE []model.CVEStructure = make([]model.CVEStructure, 0)
	var totalScore float64 = 0

	if config.LOGIC == "execute" {
		result := utils.RemoveDuplicatesFromMap(els.GatherAllDataInMap(els.CveIndex, "and", map[string]interface{}{
			"cve.descriptions.value": fmt.Sprintf("siemens %v", s.Version),
		}))
		if len(result) == 0 {
			return
		}
...
func (s *Siemens) PrintInfo() string { return model.Category.Industrial() + " | Siemens" }

func (s *Siemens) Result() model.ModuleStructure {

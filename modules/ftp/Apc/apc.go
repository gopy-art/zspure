...
func (a *Apc) SetCategory(category ...string) {
...
func (a *Apc) Filters(banner map[string]interface{}) bool {
	if banner["banner"] == nil {
		return false
	}
	if val, ok := banner["banner"].(string); ok {
		if strings.Contains(val, "FTP server") &&
			(strings.Contains(val, "APC") || strings.Contains(val, "AOS")) &&
...
func (a *Apc) DeviceScan(banner map[string]interface{}) bool {
	if val, ok := banner["banner"]; ok {
		bannerStr := fmt.Sprintf("%v", val)
		if strings.Contains(bannerStr, "Network Management Card AOS") {
			a.ExtraInformation.NewExtraInfo()
			a.ExtraInformation.SetExtraInfo("product", strings.Split(strings.Split(bannerStr, "Network Management Card AOS")[0], "220")[1])
			versionRe := regexp.MustCompile(`AOS v(\d+(?:\.\d+)*) FTP server`)
...
func (a *Apc) CveScan(els *handler.Elastic) {
...
func (a *Apc) PrintInfo() string { return model.Category.Electric() + " | Apc" }

func (a *Apc) Result() model.ModuleStructure {

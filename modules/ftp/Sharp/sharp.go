...
func (s *Sharp) Filters(banner map[string]interface{}) bool {
	if banner["banner"] != nil && 
		...
		strings.Contains(strings.ToLower(banner["banner"].(string)), strings.ToLower("220 SHARP")) {
		return true
	}
	return false
}
...
func (s *Sharp) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		result := utils.RemoveDuplicatesFromMap(els.GatherAllDataInMap(els.CveIndex, "and", map[string]interface{}{
			"cve.descriptions.value": fmt.Sprintf("%v %v %v", s.DeviceName, modeltype, s.Version),
		}))
		if len(result) == 0 {
			return
		}
		...
		url := fmt.Sprintf(model.CVE.MainResource(),
			strings.ToLower(strings.ReplaceAll(fmt.Sprintf("%v %v %v", s.DeviceName, modeltype, s.Version), " ", "%20")))
...
func (s *Sharp) PrintInfo() string {
	return model.Category.Printer() + " | Sharp"
}

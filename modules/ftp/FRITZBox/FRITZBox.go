...
func (a *FRITZBox) SetCategory(category ...string) {
	a.Category = model.Category.Router()
}

func (a *FRITZBox) SetDeviceName(device ...string) {
	a.DeviceName = "FRITZBox"
}

func (a *FRITZBox) Patterns() []map[string]interface{} {
	return []map[string]interface{}{}
}

func (a *FRITZBox) Filters(banner map[string]interface{}) bool {
...
func (a *FRITZBox) DeviceScan(banner map[string]interface{}) bool {
	if val, ok := banner["banner"]; ok {
		bannerStr := fmt.Sprintf("%v", val)
		if strings.Contains(bannerStr, "FRITZ!Box") {
			productRe := regexp.MustCompile(`(?i)^220 (.+) FTP server ready`)
			matches := productRe.FindStringSubmatch(bannerStr)
			if len(matches) > 1 {
...
func (a *FRITZBox) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), "FRITZBox%20"+a.Version)
...
func (a *FRITZBox) PrintInfo() string { return model.Category.Router() + " | FRITZBox" }

func (a *FRITZBox) Result() model.ModuleStructure {

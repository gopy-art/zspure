...
func (t *Trimble) SetDeviceName(device ...string) {
	t.DeviceName = "Trimble FTP Server"
}
...
func (t *Trimble) Filters(banner map[string]interface{}) bool {
	if banner["banner"] != nil && banner["banner"].(string) != "" && strings.Contains(banner["banner"].(string), "(Trimble)") {
		return true
	}
	return false
}
...
func (t *Trimble) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		result := utils.RemoveDuplicatesFromMap(els.GatherAllDataInMap(els.CveIndex, "and", map[string]interface{}{
			"cve.descriptions.value": fmt.Sprintf("trimble %v", t.Version),
		}))
		if len(result) == 0 {
			return
		}
		...
		url := fmt.Sprintf(model.CVE.MainResource(),
			strings.ToLower(strings.ReplaceAll(fmt.Sprintf("trimble %v", t.Version), " ", "%20")))
...
func (t *Trimble) PrintInfo() string { return model.Category.Industrial() + " | Trimble FTP Server" }

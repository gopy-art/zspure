...
func (a *GenericDsl) SetCategory(category ...string) {
	a.Category = model.Category.Router()
}

func (a *GenericDsl) SetDeviceName(device ...string) {
	a.DeviceName = "GenericDsl"
}

func (a *GenericDsl) Patterns() []map[string]interface{} {
	return []map[string]interface{}{}
}

func (a *GenericDsl) Filters(banner map[string]interface{}) bool {
...
func (a *GenericDsl) DeviceScan(banner map[string]interface{}) bool {
		...
		if strings.Contains(bannerStr, "DSL Router FTP Server") {
			...
			if len(matches) > 2 {
				...
				if matches[2] != "" {
					...
					a.ExtraInformation.SetExtraInfo("revision", matches[2])
				}
				return true
			}
		}
...
func (a *GenericDsl) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), "genericDsl%20"+a.Version)
...
func (a *GenericDsl) PrintInfo() string { return model.Category.Router() + " | GenericDsl" }

func (a *GenericDsl) Result() model.ModuleStructure {

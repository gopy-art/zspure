...
func (a *Pfsense) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "<title>pfSense - Login</title>"},
		{"result.response.body": "aria-labelledby=\"pfsense-logo\""},
	}
}
...
func (p *Pfsense) Filters(banner map[string]interface{}) bool {
	...
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if strings.Contains(val.(string), "<title>pfSense - Login</title>") &&
			strings.Contains(val.(string), "aria-labelledby=\"pfsense-logo\"") &&
...
func (p *Pfsense) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(),
			strings.ToLower(strings.ReplaceAll(fmt.Sprintf("%v", p.DeviceName), " ", "%20")))

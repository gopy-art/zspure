...
func (c *CiscoWLC) SetCategory(category ...string) {
	c.Category = model.Category.Controller()
}

func (c *CiscoWLC) SetDeviceName(device ...string) {
	c.DeviceName = "Cisco (WLC)"
}

func (a *CiscoWLC) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "<script>window.onload=function(){ url ='/webui';window.location.href=url;}</script>"},
	}
}

func (c *CiscoWLC) Filters(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if strings.Contains(val.(string), "<script>window.onload=function(){ url ='/webui';window.location.href=url;}</script>") {
			return true
		}
	}
	return false
...
func (c *CiscoWLC) DeviceScan(banner map[string]interface{}) bool {
...
func (c *CiscoWLC) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), "cisco wlc")
...
func (c *CiscoWLC) PrintInfo() string { return model.Category.Controller() + " | Cisco (WLC)" }

func (c *CiscoWLC) Result() model.ModuleStructure {

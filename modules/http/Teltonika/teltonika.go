...
func (a *Teltonika) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "<title>Teltonika Networks</title>"},
		{"result.response.body": "<meta name=robots content=noindex>"},
		{"result.response.body": "<meta name=\"robots\" content=\"noindex\">"},
	}
}
...
func (t *Teltonika) Filters(banner map[string]interface{}) bool {
	...
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if (strings.Contains(val.(string), "<title>Teltonika Networks</title>") &&
			(strings.Contains(val.(string), "<meta name=robots content=noindex>") || strings.Contains(val.(string), "<meta name=\"robots\" content=\"noindex\">"))) &&

...
func (m *MicrosoftHttpApi) SetCategory(category ...string) {
	m.Category = model.Category.WebServer()
}

func (m *MicrosoftHttpApi) SetDeviceName(device ...string) {
	m.DeviceName = "Microsoft HTTP API"
}

func (m *MicrosoftHttpApi) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.headers.server": "microsoft-httpapi"},
	}
}

func (lm *MicrosoftHttpApi) Filters(banner map[string]interface{}) bool {
...
func (m *MicrosoftHttpApi) DeviceScan(banner map[string]interface{}) bool {
...
func (m *MicrosoftHttpApi) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), "microsoft-httpapi%20"+m.Version)

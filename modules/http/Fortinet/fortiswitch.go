...
func (f *FortiSwitch) SetCategory(category ...string) {
	f.Category = model.Category.Switch()
}

func (f *FortiSwitch) SetDeviceName(device ...string) {
	f.DeviceName = "FortiSwitch"
}

func (a *FortiSwitch) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "<title>FortiSwitch - Login</title>"},
	}
}

func (f *FortiSwitch) Filters(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if strings.Contains(val.(string), "FortiSwitch - Login") &&
...
func (f *FortiSwitch) DeviceScan(banner map[string]interface{}) bool {
...
func (f *FortiSwitch) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), f.DeviceName)
...
func (f *FortiSwitch) PrintInfo() string { return model.Category.Switch() + " | FortiSwitch" }

func (f *FortiSwitch) Result() model.ModuleStructure {

...
func (s *SophosFirewall) SetCategory(category ...string) {
	s.Category = model.Category.Firewall()
}

func (s *SophosFirewall) SetDeviceName(device ...string) {
	s.DeviceName = "Sophos"
}

func (a *SophosFirewall) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "webconsole/webpages/login.jsp"},
		{"result.response.body": "<title>User Portal</title>"},
	}
}

func (s *SophosFirewall) Filters(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if (strings.Contains(val.(string), "webconsole/webpages/login.jsp") ||
			strings.Contains(val.(string), "<title>User Portal</title>")) &&
...
func (s *SophosFirewall) DeviceScan(banner map[string]interface{}) bool {
...
func (s *SophosFirewall) CveScan(els *handler.Elastic) {
...
func (s *SophosFirewall) PrintInfo() string { return model.Category.Firewall() + " | Sophos" }

func (s *SophosFirewall) Result() model.ModuleStructure {

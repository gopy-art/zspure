...
func (a *Linksys) Patterns() []map[string]interface{} {
...
func (l *Linksys) Filters(banner map[string]interface{}) bool {
	...
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if strings.Contains(val.(string), "<title>Linksys Smart Wi-Fi</title>") &&
...
func (l *Linksys) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), "linksys")
...
func (l *Linksys) PrintInfo() string { return model.Category.Router() + " | Linksys" }

...
func (m *MicrosoftWinCE) SetCategory(category ...string) {
	m.Category = model.Category.Service()
}

func (m *MicrosoftWinCE) SetDeviceName(device ...string) {
	m.DeviceName = "Microsoft WinCE"
}

func (m *MicrosoftWinCE) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.headers.server": "Microsoft-WinCE"},
		{"result.response.headers.server": "Microsoft WinCE"},
	}
}

func (lm *MicrosoftWinCE) Filters(banner map[string]interface{}) bool {
...
func (m *MicrosoftWinCE) DeviceScan(banner map[string]interface{}) bool {
	...
	if strings.Contains(banner["response"].(map[string]interface{})["headers"].(map[string]interface{})["server"].([]any)[0].(string), "/") {
		...
		re := regexp.MustCompile(`Microsoft WinCE [A-Za-z]+ v([\d.]+)`)
		matches := re.FindStringSubmatch(banner["response"].(map[string]interface{})["headers"].(map[string]interface{})["server"].([]any)[0].(string))
		if len(matches) > 1 {
			m.Version = matches[1]
		}
	}
...
func (m *MicrosoftWinCE) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), "microsoft"+"%20"+"wince%20"+m.Version)
...
func (m *MicrosoftWinCE) PrintInfo() string { return model.Category.Service() + " | Microsoft WinCE" }

func (m *MicrosoftWinCE) Result() model.ModuleStructure {

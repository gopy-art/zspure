...
func (n *NginxServer) SetCategory(category ...string) {
	n.Category = model.Category.WebServer()
}

func (n *NginxServer) SetDeviceName(device ...string) {
	n.DeviceName = "Nginx"
}

func (a *NginxServer) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "<h1>Welcome to nginx!</h1>"},
		{"result.response.body": "<title>Welcome to nginx!</title>"},
	}
}

func (n *NginxServer) Filters(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if (strings.Contains(val.(string), "<h1>Welcome to nginx!</h1>") ||
			strings.Contains(val.(string), "<title>Welcome to nginx!</title>")) &&
...
func (n *NginxServer) DeviceScan(banner map[string]interface{}) bool {
...
func (n *NginxServer) CveScan(els *handler.Elastic) {
...
func (n *NginxServer) PrintInfo() string { return model.Category.WebServer() + " | Nginx" }

func (n *NginxServer) Result() model.ModuleStructure {

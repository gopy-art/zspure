...
func (z *ZyXELVPN) SetCategory(category ...string) {
	z.Category = model.Category.VPN()
}

func (z *ZyXELVPN) SetDeviceName(device ...string) {
	z.DeviceName = "ZyXEL VPN"
}

func (a *ZyXELVPN) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "zyxel"},
		{"result.response.body": "ZyXEL"},
		{"result.response.body": "<h2 class=\"zyxel_title1\">VPN Authorize</h2>"},
		{"result.response.body": "<title>Welcome</title>"},
	}
}

func (z *ZyXELVPN) Filters(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if strings.Contains(val.(string), "<h2 class=\"zyxel_title1\">VPN Authorize</h2>") &&
			(strings.Contains(val.(string), "zyxel") || strings.Contains(val.(string), "ZyXEL")) &&
			strings.Contains(val.(string), "<title>Welcome</title>") &&
...
func (z *ZyXELVPN) DeviceScan(banner map[string]interface{}) bool {
...
func (z *ZyXELVPN) CveScan(els *handler.Elastic) {
	var CVE []model.CVEStructure = make([]model.CVEStructure, 0)
	var totalScore float64 = 0

	if config.LOGIC == "execute" {
		result := utils.RemoveDuplicatesFromMap(els.GatherAllDataInMap(els.CveIndex, "and", map[string]interface{}{"cve.descriptions.value": fmt.Sprintf("%s", z.DeviceName)}))
		if len(result) == 0 {
			return
		}
...
func (z *ZyXELVPN) PrintInfo() string { return model.Category.VPN() + " | ZyXEL VPN" }

func (z *ZyXELVPN) Result() model.ModuleStructure {

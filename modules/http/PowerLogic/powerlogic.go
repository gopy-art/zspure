...
func (p *PowerLogic) SetCategory(category ...string) {
	p.Category = model.Category.Electric()
}

func (p *PowerLogic) SetDeviceName(device ...string) {
	p.DeviceName = "PowerLogic"
}

func (a *PowerLogic) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "PowerLogic ION 8650"},
	}
}

func (p *PowerLogic) Filters(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if strings.Contains(val.(string), "PowerLogic ION 8650") &&
...
func (p *PowerLogic) DeviceScan(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	...
	if val, ok := banner["response"].(map[string]interface{})["headers"]; ok {
		if server, sok := val.(map[string]interface{})["server"].([]any); sok {
			p.ExtraInformation.SetExtraInfo("server", server[0].(string))
		}
	}
	if val, ok := banner["response"].(map[string]interface{})["headers"]; ok {
		if pl, pok := val.(map[string]interface{})["x_powered_by"].([]any); pok {
			p.ExtraInformation.SetExtraInfo("programming_language", pl[0].(string))
		}
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		...
		// Find the `TAG:title` tag and get its text it will include version
		text := doc.Find("title").Text()
		words := strings.Fields(text)

		if len(words) >= 3 {
			p.Version = words[1] + words[2]
			return true
		}
	}
	return false
...
func (p *PowerLogic) CveScan(els *handler.Elastic) {
...
func (p *PowerLogic) PrintInfo() string { return model.Category.Electric() + " | PowerLogic" }

func (p *PowerLogic) Result() model.ModuleStructure {

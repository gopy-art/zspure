...
func (s *SonicWall) SetCategory(category ...string) {
	s.Category = model.Category.Firewall()
}

func (s *SonicWall) SetDeviceName(device ...string) {
	s.DeviceName = "SonicWall"
}

func (a *SonicWall) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "<title>SonicWall - Authentication</title>"},
		{"result.response.body": "<meta name=\"SonicWall Administrator\" content=\"Copyright 2004 (c) SonicWall, Inc. All rights reserved.\">"},
		{"result.response.body": "top.location.href=window.location.href;"},
	}
}

func (s *SonicWall) Filters(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if (strings.Contains(val.(string), "<title>SonicWall - Authentication</title>") &&
			strings.Contains(val.(string), "<meta name=\"SonicWall Administrator\" content=\"Copyright 2004 (c) SonicWall, Inc. All rights reserved.\">") &&
			strings.Contains(val.(string), "top.location.href=window.location.href;")) &&
...
func (s *SonicWall) DeviceScan(banner map[string]interface{}) bool {
...
func (s *SonicWall) CveScan(els *handler.Elastic) {
...
func (s *SonicWall) PrintInfo() string { return model.Category.Firewall() + " | SonicWall" }

func (s *SonicWall) Result() model.ModuleStructure {

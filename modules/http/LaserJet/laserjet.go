...
func (l *LaserJet) SetCategory(category ...string) {
	l.Category = model.Category.Printer()
}

func (l *LaserJet) SetDeviceName(device ...string) {
	l.DeviceName = "LaserJet"
}

func (a *LaserJet) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "HP LaserJet Pro MFP"},
		{"result.response.body": "HP Color LaserJet MFP"},
		{"result.response.body": "Xerox(R) C235 Color MFP"},
		{"result.response.body": "HP LaserJet CM1415fnw"},
	}
}

func (l *LaserJet) Filters(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if (strings.Contains(val.(string), "HP LaserJet Pro MFP") ||
			strings.Contains(val.(string), "HP Color LaserJet MFP") ||
			strings.Contains(val.(string), "Xerox(R) C235 Color MFP") ||
			strings.Contains(val.(string), "HP LaserJet CM1415fnw")) &&
...
func (l *LaserJet) DeviceScan(banner map[string]interface{}) bool {
	if banner["response"] == nil {
		return false
	}
	...
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		...
		text := doc.Find("td.mastheadTitle").Text()
...
func (l *LaserJet) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), "laserjet%20"+l.Version)
...
func (l *LaserJet) PrintInfo() string { return model.Category.Printer() + " | LaserJet" }

func (l *LaserJet) Result() model.ModuleStructure {

...
func (x *XunboPepLink) SetCategory(category ...string) {
	x.Category = model.Category.Firewall()
}

func (x *XunboPepLink) SetDeviceName(device ...string) {
	x.DeviceName = "Xunbo Peplink NG VPN"
}

func (x *XunboPepLink) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.headers.www_authenticate": "XUNBO PEPLINK NG VPN/Firewall"},
	}
}

func (x *XunboPepLink) Filters(banner map[string]interface{}) bool {
	...
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		...
		if val, ok := banner["response"].(map[string]interface{})["headers"]; ok {
			if pl, pok := val.(map[string]interface{})["www_authenticate"].([]any); pok {
				if strings.Contains(strings.ToUpper(pl[0].(string)), strings.ToUpper("XUNBO PEPLINK NG VPN/Firewall")) {
					return true
				}
			}
		}
...
func (x *XunboPepLink) DeviceScan(banner map[string]interface{}) bool {
...
func (x *XunboPepLink) CveScan(els *handler.Elastic) {
	...
	if config.LOGIC == "execute" {
		...
		url := fmt.Sprintf(model.CVE.MainResource(), 
				strings.ReplaceAll(strings.ToLower(x.DeviceName), " ", "%20"))
...
func (x *XunboPepLink) PrintInfo() string { return model.Category.Firewall() + " | Xunbo Peplink NG VPN" }

func (x *XunboPepLink) Result() model.ModuleStructure {

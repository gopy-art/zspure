...
func (s *SecPath) SetCategory(category ...string) {
	s.Category = model.Category.Firewall()
}

func (s *SecPath) SetDeviceName(device ...string) {
	s.DeviceName = "SecPath"
}

func (a *SecPath) Patterns() []map[string]interface{} {
	return []map[string]interface{}{
		{"result.response.body": "firewall"},
		{"result.response.body": "Firewall"},
		{"result.response.body": "secpath"},
		{"result.response.body": "SecPath"},
	}
}

func (s *SecPath) Filters(banner map[string]interface{}) bool {
	...
	if val, ok := banner["response"].(map[string]interface{})["body"]; ok {
		if (strings.Contains(val.(string), "firewall") || strings.Contains(val.(string), "Firewall")) &&
			(strings.Contains(val.(string), "secpath") || strings.Contains(val.(string), "SecPath")) &&
...
func (s *SecPath) DeviceScan(banner map[string]interface{}) bool {
...
func (s *SecPath) CveScan(els *handler.Elastic) {
	var CVE []model.CVEStructure = make([]model.CVEStructure, 0)
	var totalScore float64 = 0

	if config.LOGIC == "execute" {
		result := utils.RemoveDuplicatesFromMap(els.GatherAllDataInMap(els.CveIndex, "and", map[string]interface{}{"cve.descriptions.value": fmt.Sprintf("%s", s.DeviceName)}))
		if len(result) == 0 {
			return
		}
...
func (s *SecPath) PrintInfo() string { return model.Category.Firewall() + " | SecPath" }

func (s *SecPath) Result() model.ModuleStructure {

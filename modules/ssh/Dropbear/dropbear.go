...
func (d *Dropbear) SetCategory(category ...string) {
	d.Category = model.Category.Service()
}

func (d *Dropbear) SetDeviceName(device ...string) {
	d.DeviceName = "Dropbear"
}

func (d *Dropbear) Patterns() []map[string]interface{} {
	return []map[string]interface{}{}
}

func (d *Dropbear) Filters(banner map[string]interface{}) bool {
	if banner["server_id"] == nil {
		return false
	}
	if val, ok := banner["server_id"].(map[string]interface{}); ok && val != nil {
		if v, okk := val["raw"].(string); okk && strings.Contains(strings.ToLower(v), "dropbear") {
			return true
		}
	}
	return false
...
func (d *Dropbear) DeviceScan(banner map[string]interface{}) bool {
	...
	re := regexp.MustCompile(`dropbear_([\d.]+)`)
...
func (d *Dropbear) CveScan(els *handler.Elastic) {
...
func (d *Dropbear) PrintInfo() string { return model.Category.Service() + " | Dropbear" }

func (d *Dropbear) Result() model.ModuleStructure {
